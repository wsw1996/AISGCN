...
def ade(predAll,targetAll,count_):
    ...
    sum_all = 0 
    for s in range(All):
        pred = np.swapaxes(predAll[s][:,:count_[s],:],0,1)   #前两个维度交换
...
def bivariate_loss(V_pred,V_trgt):  #12,57,5   12,57,2
    ...
    normx = V_trgt[:,:,0]- V_pred[:,:,0]  #目标值-均值  x-均值
    normy = V_trgt[:,:,1]- V_pred[:,:,1]            # y-均值

    sx = torch.exp(V_pred[:,:,2]) #sx  得到的是x的标准差
    sy = torch.exp(V_pred[:,:,3]) #sy   得到的是y的标准差
    corr = torch.tanh(V_pred[:,:,4]) #corr 表示xy的相关系数
    ...
    # Final PDF calculation# 最终距离计算 Final PDF calculation
    result = result / denom
